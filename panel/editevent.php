...
// ustawienie zmiennej sesji dla id eventu
...
//wczytanie danych eventu do pól
$sql = "SELECT Name, Description, PlaceID, TypeID, Date FROM events WHERE EventID = ".$_GET['id'];

$result = $link->query($sql);
    while ($rekord=$result->fetch_object()) {
    $eventName = $rekord->Name;
    $eventDesc = $rekord->Description;
    $eventPlace = $rekord->PlaceID;
    $eventType = $rekord->TypeID;
...
//dodawanie event'u
    ...
    function update_func()
    {
        ...
        $sql = "UPDATE events SET Name='?', description='?', PlaceID=?, TypeID=? Date='?'";
...
<head>
    ...
    <div class="col-md-8 mx-auto">
        <div class="card">
            ...
            <div class="card-body">
                <form method="post" action="update.php" enctype="multipart/form-data" id='event_form'>
                    <div class="form-group row">
                        <label for="name" class="col-md-4 col-form-label text-md-right">Nazwa</label>

                        <div class="col-md-6">
                            <input id="name" type="text" class="form-control" name="name" required autocomplete="on" value="<?php echo htmlspecialchars($eventName)?>" autofocus>
                        </div>
                    </div>
                            ...
                            <select id='type' name='type' class='custom-select' required>
                                ...
                                <?php
                                    while(mysqli_stmt_fetch($types)){
                                        ...
                                        if ($eventType==$typeID) echo 'selected="selected"';
                                        echo ">".$type."</option>";
                                    }
                                ?>
                            </select>
                            ...
                            <select id='place' name='place' class='custom-select' required>
                                ...
                                <?php
                                    while(mysqli_stmt_fetch($places)){
                                        ...
                                        if ($eventPlace==$placeID) echo 'selected="selected"';
                                        echo ">".$postal." ".$city." / ".$street." ".$number."</option>";
                                    }
                                ?>
                            </select>
                ...
                <div class="row">
                    ...
                    <div class="col-md-3 text-right">
                        ...
                        <button type="submit" class='btn btn-success' form='event_form'>Dodaj</button>
                    </div>
                </div>
            </div>
        </div>
